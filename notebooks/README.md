...
- **`rapport_analytique.ipynb`** : **le notebook livrable** - sections 1-7 : analyse sur les données nettoyées (`../data/*.csv`) en pandas/networkx (répartition géographique, couverture Wikidata, réseau d'influence, mouvements). Section 8 : les 5 requêtes métier rejouées en **Cypher réel** contre AuraDB, en important directement `database/scripts/queries_metier.py` (le code de Rémi, non dupliqué). Section 9 : synthèse comparant les deux. Ne nécessite pas `joconde.csv`, mais la section 8 nécessite un `database/.env` valide (voir `database/.env.example`) - sans lui, n'exécuter que les sections 1-7.
