...
- un rôle **lecture seule** pour le backend (Hono/Node) de Conambot Nguessan (qui n'a besoin
...
3. **Le backend (Railway)** doit lire `NEO4J_URI/USERNAME/PASSWORD`
   depuis des **variables d'environnement Railway**, jamais commitées dans le
   repo - même règle que `.env` en local.
4. **CORS côté backend** : n'autoriser que l'origine du frontend déployé
...
5. **Ne jamais exposer le driver `neo4j` côté navigateur** - rappel de
   ...
   backend Node (Hono) est obligatoire.
